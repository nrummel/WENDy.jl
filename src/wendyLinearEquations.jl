...
function _L₁!(
    ...
    ∂Lⱼ::AbstractArray{<:Real, 4}, 
    eⱼ::AbstractVector{<:Real}
)
    for j = 1:J 
        eⱼ .= 0
        eⱼ[j] = 1
...
function _Hm!(
    ...
    @inbounds for j = 1:J
        ...
        for i = j:J 
            ...
            @views prt1 = dot(∇r[:,j], S⁻¹∇r[:,i])             # ∂ⱼrᵀS⁻¹∂ᵢr
            @views prt2 = - 2*dot(S⁻¹∇r[:,i], ∇S[:,:,j], S⁻¹r) # 2∂ᵢrᵀ∂ⱼS⁻¹r = -(S⁻¹∂ᵢr)ᵀ∂ⱼSS⁻¹r
            @views prt3 = 2*dot(S⁻¹r, ∂ᵢSS⁻¹∂ⱼS, S⁻¹r)         #             + 2(S⁻¹r)ᵀ∂ᵢSS⁻¹∂ⱼSS⁻¹r
            # Put everything together
            H[j,i] = 1/2*(2*(prt1+prt2) + prt3+prt5)
        end
    end
