...
display(_p_sols[:tr_optim])
##
save_dir = "/Users/user/Documents/School/WSINDy/NonLinearWENDyPaper/fig/goodwin"
...
open(joinpath(save_dir, "$(ex.name)_caption.tex"), "w") do f
    s = """
    The plots above show the solutions to the $(ex2Disp[ex.name]) differential equations. The plot on the left show the solution with the intial guessed parameters $(L"w_0") and the plot of the right shows the approximated parameteres form minimizing the negative log-likelihood with the trust region solver provided by Optim.jl. The true solution, $(L"U^*"), is shown with the dashed lines. The noisey data, $(L"U"), is displayed with the markers, and the approximation, $(L"\hat{U}"), using the specified parameters are shown with the solid lines. 
