...
open(joinpath(save_dir, "$(ex.name)_caption.tex"), "w") do f
    ...
    Both FSNLS and the Negative Log-Likelihood methods failed in this case. FSNLS appears to have made good progress, but then instead of exitting, it continued to try to improve its answer only to then ultimately return no answer. The best approach was to use the IRWLS, which converges to a decent answer quickly.
"""
    write(f, s)
end
##
