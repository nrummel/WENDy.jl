...
This package provides convenience code to solve OE-LS. See the example of using OE-LS 

One also can use Monte Carlo approaches, but unfortunately the underlying sampling will depend on direct simulation of the system. This causes these methods to be much more computationally expensive compared to the weak-form methods implemented here or OE-LS
## Acknowledgements 
...
This code base is developed primarily by Nicholas Rummel who can be reached via [email](mailto:nicholas.rummel@colorad.edu). If you are interested in contributing please reach out directly. For support, we suggest you open a [GitHub issue](https://github.com/nrummel/WENDy.jl/issues).